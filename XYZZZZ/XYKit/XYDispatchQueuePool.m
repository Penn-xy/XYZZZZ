...
static XYDispatchContext *XYDispatchContextCreate(const char *name,
                                                  uint32_t queueCount,
                                                  NSQualityOfService qos) {
    XYDispatchContext *context = calloc(1, sizeof(XYDispatchContext));
...
static void XYDispatchContextRelease(XYDispatchContext *context) {
    if (!context) return;
    if (context->queues) {
...
static dispatch_queue_t XYDispatchContextGetQueue(XYDispatchContext *context) {
...
static XYDispatchContext *XYDispatchContextGetForQOS(NSQualityOfService qos) {
    static XYDispatchContext *context[5] = {0};
    switch (qos) {
        case NSQualityOfServiceUserInteractive: {
            ...
            dispatch_once(&onceToken, ^{
                ...
                context[0] = XYDispatchContextCreate("com.ibireme.xykit.user-interactive", count, qos);
            });
            return context[0];
        } break;
        case NSQualityOfServiceUserInitiated: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[1] = XYDispatchContextCreate("com.ibireme.xykit.user-initiated", count, qos);
            });
            return context[1];
        } break;
        case NSQualityOfServiceUtility: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[2] = XYDispatchContextCreate("com.ibireme.xykit.utility", count, qos);
            });
            return context[2];
        } break;
        case NSQualityOfServiceBackground: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[3] = XYDispatchContextCreate("com.ibireme.xykit.background", count, qos);
            });
            return context[3];
        } break;
        ...
        default: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[4] = XYDispatchContextCreate("com.ibireme.xykit.default", count, qos);
            });
            return context[4];
        } break;
    }
...
@implementation XYDispatchQueuePool {
    @public
    XYDispatchContext *_context;
}

- (void)dealloc {
    if (_context) {
        XYDispatchContextRelease(_context);
        _context = NULL;
    }
}

- (instancetype)initWithContext:(XYDispatchContext *)context {
    self = [super init];
...
- (instancetype)initWithName:(NSString *)name queueCount:(NSUInteger)queueCount qos:(NSQualityOfService)qos {
    ...
    self = [super init];
    _context = XYDispatchContextCreate(name.UTF8String, (uint32_t)queueCount, qos);
...
- (dispatch_queue_t)queue {
    return XYDispatchContextGetQueue(_context);
}

+ (instancetype)defaultPoolForQOS:(NSQualityOfService)qos {
    switch (qos) {
        case NSQualityOfServiceUserInteractive: {
            static XYDispatchQueuePool *pool;
            static dispatch_once_t onceToken;
            dispatch_once(&onceToken, ^{
                pool = [[XYDispatchQueuePool alloc] initWithContext:XYDispatchContextGetForQOS(qos)];
            });
            return pool;
        } break;
        case NSQualityOfServiceUserInitiated: {
        ...
        default: {
            ...
            static dispatch_once_t onceToken;
            dispatch_once(&onceToken, ^{
                pool = [[XYDispatchQueuePool alloc] initWithContext:XYDispatchContextGetForQOS(NSQualityOfServiceDefault)];
            });
            return pool;
        } break;
    }
...
dispatch_queue_t XYDispatchQueueGetForQOS(NSQualityOfService qos) {
    return XYDispatchContextGetQueue(XYDispatchContextGetForQOS(qos));
}
