...
@implementation _XYXMLDictionaryParser {
...
+ (NSDictionary *)dictionaryWithXML:(id)xml {
    _XYXMLDictionaryParser *parser = nil;
    if ([xml isKindOfClass:[NSString class]]) {
        parser = [[_XYXMLDictionaryParser alloc] initWithString:xml];
    } else if ([xml isKindOfClass:[NSData class]]) {
        parser = [[_XYXMLDictionaryParser alloc] initWithData:xml];
    }
...
@implementation NSMutableDictionary (XYAdd)
