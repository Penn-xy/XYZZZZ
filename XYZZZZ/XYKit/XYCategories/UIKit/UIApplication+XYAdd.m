...
@implementation _XYUIApplicationNetworkIndicatorInfo
@end
...
- (BOOL)isPirated {
    ...
    if (![self _xy_fileExistInMainBundle:@"_CodeSignature"]) {
        return YES;
    }
    
    if (![self _xy_fileExistInMainBundle:@"SC_Info"]) {
        return YES;
    }
...
- (BOOL)_xy_fileExistInMainBundle:(NSString *)name {
...
XYSYNTH_DYNAMIC_PROPERTY_OBJECT(networkActivityInfo, setNetworkActivityInfo, RETAIN_NONATOMIC, _XYUIApplicationNetworkIndicatorInfo *);
...
- (void)_changeNetworkActivityCount:(NSInteger)delta {
    @synchronized(self){
        dispatch_async_on_main_queue(^{
            _XYUIApplicationNetworkIndicatorInfo *info = [self networkActivityInfo];
            if (!info) {
                info = [_XYUIApplicationNetworkIndicatorInfo new];
