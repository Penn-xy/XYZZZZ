...
static int _XYKeyboardViewFrameObserverKey;


@interface _XYKeyboardViewFrameObserver : NSObject
...
@implementation _XYKeyboardViewFrameObserver {
...
- (void)addToKeyboardView:(UIView *)keyboardView {
    ...
    if (_keyboardView) {
        ...
        objc_setAssociatedObject(_keyboardView, &_XYKeyboardViewFrameObserverKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    _keyboardView = keyboardView;
    if (keyboardView) {
        [self addFrameObserver];
    }
    objc_setAssociatedObject(keyboardView, &_XYKeyboardViewFrameObserverKey, self, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
+ (instancetype)observerForView:(UIView *)keyboardView {
    ...
    return objc_getAssociatedObject(keyboardView, &_XYKeyboardViewFrameObserverKey);
}
...
- (void)_initFrameObserver {
    ...
    _XYKeyboardViewFrameObserver *observer = [_XYKeyboardViewFrameObserver observerForView:keyboardView];
    if (!observer) {
        observer = [_XYKeyboardViewFrameObserver new];
...
- (void)_notifyAllObservers {
    ...
    if (!CGRectEqualToRect(trans.toFrame, _fromFrame)) {
        for (id<XYKeyboardObserver> observer in _observers.copy) {
