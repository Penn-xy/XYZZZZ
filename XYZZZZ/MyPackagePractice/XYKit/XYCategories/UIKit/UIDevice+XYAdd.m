...
static uint64_t yy_net_counter_get_by_type(xy_net_interface_counter *counter, XYNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & XYNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & XYNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & XYNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & XYNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & XYNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & XYNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}

static xy_net_interface_counter yy_get_net_interface_counter() {
    ...
    xy_net_interface_counter counter = {0};
...
- (uint64_t)getNetworkTrafficBytes:(XYNetworkTrafficType)types {
    xy_net_interface_counter counter = yy_get_net_interface_counter();
